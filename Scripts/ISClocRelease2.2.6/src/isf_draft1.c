 ...
 */
    ...
    for (nstas = 0, i = 0; i < ep->numphas; i++) {
	    ...
	    if (isf == 1 || isf == 2){
		// If arrID is correctly define in ISF(1/2/2.1) as an integer, the reading id (rdid) can be set to this
		// If arrID is defined differently this may cause difficulties. JH/TG Oct 2023
		p[i].rdid = p[i].arrid; 
            }
            else{
		p[i].rdid = rdid++;
    ...
    fprintf(stdout, "!ep->numrd = %f rdid = %f", ep->numrd, rdid);
