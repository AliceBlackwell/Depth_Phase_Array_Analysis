...
{
    char sql[2048], psql[2048];
    int i, m, row, prev_rdid, rdid, numrd = 0;
    ...
    char amptype[10], ampchan[10], prista[STALEN];
    PGresult *res_set = (PGresult *)NULL;
/*
 *  Get phases ISC associated to the prime hypocentre
 */
    ...
    if ((res_set = PQexec(conn, psql)) == NULL)
        ...
        if (numphas != ep->numphas) {
            fprintf(errfp, "get_pha: unexpected number of phases!\n");
            fprintf(errfp, "    evid=%d prime=%d epnumphas=%d numphas=%d\n",
                    ep->evid, ep->prime, ep->numphas, numphas);
            fprintf(logfp, "get_pha: unexpected number of phases!\n");
        ...
        for (i = 0; i < numphas; i++) {
            ...
            strcpy(p[i].agency, PQgetvalue(res_set, i, 14));
            if (PQgetisnull(res_set, i, 15))      { p[i].delta = NULLVAL; }
            else         { p[i].delta = atof(PQgetvalue(res_set, i, 15)); }
            strcpy(p[i].phase, PQgetvalue(res_set, i, 16));
            strcpy(w, PQgetvalue(res_set, i, 17));
    ...
    PQclear(res_set);
/*
 *  Get station coordinates for each reading
 *     consider initial phase in a reading
 *     accounts for various operational periods
 *     site.net is NULL for registered stations!
 */
    sprintf(psql, "select q.rdid, coalesce(s.prista, s.sta),      \
                         s.lat, s.lon, s.elev                     \
                    from %ssite s,                                \
                         (select p.rdid, p.sta,                   \
                          min(coalesce(p.day, '3000-01-01')) as minday  \
                            from %sphase p, %sassociation a, %sreport r \
                           where hypid = %d                       \
                             and a.author = '%s'                  \
                             and a.deprecated is NULL             \
                             and a.phid = p.phid                  \
                             and (p.deprecated is NULL or         \
                                  p.deprecated = 'M')             \
                             and p.reporter = r.repid             \
                           group by p.rdid, p.sta) q              \
                   where s.sta = q.sta                            \
                     and s.open_date <= q.minday                  \
                     and s.close_date > q.minday                  \
                   order by q.rdid, q.sta",
            indb, indb, indb, indb, ep->prime, in_agency);
    dropspace(psql, sql);
    if (verbose > 2) fprintf(logfp, "            get_pha: %s\n", sql);
    if ((res_set = PQexec(conn, psql)) == NULL)
        pgsql_error("get_pha: stations");
    else {
        numrd = PQntuples(res_set);
        if (numrd != ep->numrd) {
            fprintf(errfp, "get_pha: unexpected number of readings!\n");
            fprintf(errfp, "    evid=%d prime=%d epnumrd=%d numrd=%d\n",
                    ep->evid, ep->prime, ep->numrd, numrd);
            fprintf(logfp, "get_pha: unexpected number of readings!\n");
            fprintf(logfp, "    evid=%d prime=%d epnumrd=%d numrd=%d\n",
                    ep->evid, ep->prime, ep->numrd, numrd);
            PQclear(res_set);
            return 1;
        }
        for (row = 0; row < numrd; row++) {
            rdid = atoi(PQgetvalue(res_set, row, 0));
            strcpy(prista, PQgetvalue(res_set, row, 1));
            lat = atof(PQgetvalue(res_set, row, 2));
            lon = atof(PQgetvalue(res_set, row, 3));
            if (PQgetisnull(res_set, row, 4))        { elev = 0.; }
            else      { elev = atof(PQgetvalue(res_set, row, 4)); }
            for (i = 0; i < numphas; i++) {
                if (rdid == p[i].rdid) {
                    strcpy(p[i].prista, prista);
                    p[i].sta_lat = lat;
                    p[i].sta_lon = lon;
                    p[i].sta_elev = elev;
                }
