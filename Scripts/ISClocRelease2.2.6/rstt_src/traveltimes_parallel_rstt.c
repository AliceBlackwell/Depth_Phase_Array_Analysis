 ...
 */
        ...
        depth = sp->depth;
        slat = DEG_TO_RAD * pp->sta_lat;
        slon = DEG_TO_RAD * pp->sta_lon;
        elev = -pp->sta_elev / 1000.;
        ...
        if (slbm_shell_createGreatCircle(phase, &lat, &lon, &depth,
                                        &slat, &slon, &elev)) {
            if (verbose) {
                fprintf(logfp, "        read_ttime: no GreatCircle for %s! ",
                        pp->phase);
 ...
 */
        ...
        if (verbose > 4)
            fprintf(logfp, "sta=%-6s phase=%-8s delta=%.2f tt=%.3f dtdd=%.3f dtdh=%.3f\n",
                    pp->sta, phase, pp->delta, ttim, dtdd, dtdh);
    }
#else
/*
 ...
 */
    if (use_RSTT_PgLg) {
        if (!(streq(pp->phase, "Pg") || streq(pp->phase, "Lg") ||
              streq(pp->phase, "Pn") || streq(pp->phase, "Sn") ||
              streq(pp->phase, "Sg") ||
...
/*
 *  if crustal phase, it must be the first-arriving phase
 */
    if (streq(pp->phase, "Pg") && !pp->firstP)
        return 0;
    if (streq(pp->phase, "Pb") && !pp->firstP)
        return 0;
    if ((streq(pp->phase, "Sg") || streq(pp->phase, "Lg")) && !pp->firstS)
        return 0;
    if (streq(pp->phase, "Sb") && !pp->firstS)
        return 0;
    return 1;
}
